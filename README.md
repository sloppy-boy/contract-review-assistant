...
除一次性 LangGraph 推理黑板外，系统将可恢复、可审计的事实保存为 **Review Run**：

- 每次在线审查都有唯一运行 ID、阶段进度、失败原因和不可变生命周期事件；服务重启后仍可查询。
- 报告写入法规语料快照（内容哈希、版本、条目数、核验状态与来源），使“本次依据哪一版法条”可复现。
- 高危、或没有直接法条依据的发现默认进入“待人工复核”；法务可采纳、修改后采纳、驳回或升级会审。裁决及理由持久化为审计事件，不覆盖模型原始报告。
- 默认运行时 Adapter 为 SQLite（`data/review_runs.db`，已忽略提交）；业务仅依赖 `ReviewRunStore` 接口，后续可替换为 Postgres。

在线报告页面仅对真实在线运行开放人工裁决；离线演示报告保持只读，避免把展示数据混入审计记录。

## 生产部署

```bash
# 生产环境必须提供身份配置；API 模型密钥仍从 .env / 运行环境读取
docker compose --env-file .env up --build
```

容器以非 root 用户运行，审查运行、反馈与 trace 保存到 Docker 卷。`APP_ENV=production` 时，平台指标和反馈 API 需要 `X-API-Key`；不要把 `.env`、`settings.json`、运行时数据库或合同原文提交到仓库。

供应商模型密钥在“设置”页面保存到 Docker 数据卷；容器重建、重启后仍会保留。首次迁移到本版本时，需要在设置页重新填写一次已有的供应商 Key。

生产部署还应通过运行环境注入 `CRA_API_KEYS`，其值为 API key 到 `{subject, tenantId, role}` 的 JSON 映射；不要把该值写进 compose 文件或仓库。
